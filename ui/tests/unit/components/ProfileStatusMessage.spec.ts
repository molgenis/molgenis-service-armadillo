...
describe("Installing container progress", () => {
  ...
  beforeEach(() => {
    wrapper = mount(ProfileStatusMessage, {
      props: {
  ...
  test("renders only when status indicates an install", async () => {
    // Initially renders
    expect(wrapper.text()).toContain("Installing container 'MyProfile'");
