      ...
      <div class="filter float-end">
        <div class="row">
          <div class="col">
            ...
            >
              <i class="bi bi-caret-down-fill" v-if="isFilterCollapsed"></i
              ><i class="bi bi-caret-up-fill" v-else></i> Filter
        ...
        <div class="row">
          <div class="col">
            <div
              class="text-sm-start border rounded p-2"
      ...
      <table class="table">
        ...
        <tbody>
          ...
          >
            <td>{{ key }}</td>
            <td>
              {{ getEmxDataType(value.type, value.levels) }}<br />
              <span class="badge text-bg-secondary">{{ value.type }}</span>
            </td>
            <td>
              {{ value.missing }} ({{ getPercentageOfMissing(value.missing) }}%)
            </td>
            <td v-if="typeOptions.includes('BINARY')">
              {{ value.levels?.join(", ") }}
            </td>
          </tr>
        </tbody>
...
export default defineComponent({
  ...
  data(): {
    ...
    missingFilterValue: null | number;
    selectedType: string;
    columnFilter: string;
  } {
    return {
    ...
    typeOptions() {
      ...
      if (this.metadata) {
        Object.values(this.metadata).forEach(
          (element: { type: string; missing: string; levels: string[] }) => {
  ...
  methods: {
    ...
    getPercentageOfMissing(missingString: string): number {
      ...
      return Number(
        toPercentage(Number(missingInfo[0]), Number(missingInfo[1])).toFixed(1)
      );
    },
  },
});
