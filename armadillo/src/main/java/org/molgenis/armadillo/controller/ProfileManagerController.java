...
public class ProfileManagerController {
    public static final String ARMADILLO_PROFILE
            = "org.molgenis.armadillo.profile";
    ...
    public List<ProfileDefinition> listProfiles(Principal principal) {
        ...
        result.addAll(
                dockerClient.listContainersCmd().exec().stream().filter(container -> container.getLabels().containsKey(ARMADILLO_PROFILE)).map(container -> {
    ...
    @PreAuthorize("hasRole('ROLE_SU')")
    public Map<String, String> addProfile(Principal principal, @RequestParam String name, @RequestParam String image, @RequestParam String port) throws InterruptedException {
        //load the image
        ...
        CreateContainerResponse container
                ...
                .withLabels(Map.of("com.docker.compose.project", "armadillo-profiles", ARMADILLO_PROFILE, name))
                //mapping the port
                .withPortBindings(PortBinding.parse(port + ":6311"))
                //mapping the name
                .withName(name).exec();
        dockerClient.startContainerCmd(container.getId()).exec();
        this.auditEventPublisher.audit(principal, "Created armadillo profile: ", Map.of("name", name, "image", image, "port", port));
        ...
        dockerClient.listContainersCmd().exec().forEach(
                container -> {
                    ...
                    if (container.getLabels().get(ARMADILLO_PROFILE).equals(profileName)) {
                        ...
                        this.auditEventPublisher.audit(principal, "Deleted profile", Map.of("profileName", profileName));
