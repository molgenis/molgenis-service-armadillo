...
assign_exposome_resources <- function(){

  cli_alert_info("Assigning exposome resources")
  datashield.assign.resource(conns, symbol = 'exposures', resource = list(armadillo = 'EXPOSOME.exposures'))
  datashield.assign.expr(conns, symbol = "exposures", expr = quote(as.resource.data.frame(exposures)))

  datashield.assign.resource(conns, symbol = 'phenotypes', resource = list(armadillo = 'EXPOSOME.phenotypes'))
  datashield.assign.expr(conns, symbol = "phenotypes", expr = quote(as.resource.data.frame(phenotypes)))

  datashield.assign.resource(conns, symbol = 'description', resource = list(armadillo = 'EXPOSOME.description'))
...
verify_plot_missings_names <- function() {
  ...
  verify_output(function_name = ds_function_name, object = names(missing_plot$pooled),
                expected = c("data", "layers", "scales", "mapping", "theme", "coordinates", "facet", "plot_env", "labels"),
                fail_msg = xenon_fail_msg$clt_list_names)
}
